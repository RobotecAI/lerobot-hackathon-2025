...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        IncludeLaunchDescription(
            ...
            launch_arguments = {
                'params_file': str(pathlib.Path(__file__).parent.absolute().joinpath(f'navigation_params_{ros_distro}.yaml'))
            }.items()
        ),
        Node(
            package='rviz2',
            ...
            output='screen',
            arguments=[
                '-d', str(pathlib.Path(__file__).parent.absolute().joinpath('config.rviz'))
            ]
        ),
    ])
